...
function RespondPageContent() {
  ...
  const [isLooping, setIsLooping] = useState(false) // Loop only enabled when user clicks the button
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      const handleAudioEnded = () => {
        ...
        // Only auto-replay if:
        // 1. Loop is enabled
        // 2. User has manually played the audio at least once
        // 3. Using generated audio (not device voice)
        // 4. Audio is ready
        if (audioRef.current === audio && isLooping && userInitiatedPlayRef.current && voiceMode === 'generated' && audioStatus === 'ready') {
